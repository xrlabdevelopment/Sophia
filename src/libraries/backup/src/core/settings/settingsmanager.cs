...
namespace Sophia.Deprecated
{
    public class SettingsManager : IObserver
    {
        ...
        public SettingsManager(string dataPath, Logger logger = null)
        {
            this.logger = logger;
            this.cached_settings = new Dictionary<string, Settings>();
            ...
            if (!Directory.Exists(dataPath))
            {
                if (logger != null)
                {
                    logger.log("Directory: " + dataPath + " does not exist");
                    logger.log("Creating: " + dataPath);
                }
            ...
            where T : Settings, new()
        {
            ...
            //
            string path = Path.Combine(data_path, setting_name + FileExtensions.toString(FileExtension.XML));
            if (!File.Exists(path))
                return settings;
        ...
        public void resetSettings()
        {
            ...
            string[] savefiles = Directory.GetFiles(data_path, "*" + FileExtensions.toString(FileExtension.XML));
        ...
        {
            System.Diagnostics.Debug.Assert(cached_settings.ContainsKey(settingName), "Setting should be available here!");

            Serializer serializer = new Serializer(cached_settings[settingName].XMLObject);
            using (StreamWriter file_writer = new StreamWriter(Path.Combine(data_path, settingName + FileExtensions.toString(FileExtension.XML))))
            {
